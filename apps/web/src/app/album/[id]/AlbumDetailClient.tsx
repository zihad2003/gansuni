...
import { EXPANDED_ALBUMS, EXPANDED_TRACKS, formatDuration } from '@gansuni/shared'
import { useAudioPlayer } from '@/store/useAudioPlayer'
import { Sidebar } from '@/components/Sidebar'
import { HeaderNav } from '@/components/HeaderNav'

export default function AlbumDetailClient(): ReactNode {
  ...
  const albumTracks = EXPANDED_TRACKS.filter((t) => t.albumId === album.id)
  const tracksToPlay = albumTracks.length > 0 ? albumTracks : EXPANDED_TRACKS.slice(0, 5)
        ...
        <section className="grid lg:grid-cols-[260px_1fr] gap-6 lg:gap-8">
          ...
          <div className="min-w-0">
            ...
            <div className="space-y-6">
              {/* ALBUM BANNER */}
              <div className="relative overflow-hidden rounded-3xl p-6 sm:p-8 bg-gradient-to-r from-purple-900/60 via-neutral-900 to-emerald-900/60 border border-white/15 flex flex-col sm:flex-row items-start sm:items-center gap-6 shadow-2xl">
                <div className="relative w-36 h-36 sm:w-44 sm:h-44 rounded-2xl overflow-hidden shadow-2xl flex-shrink-0">
                  <Image src={album.coverArtUrl} alt={album.title} fill sizes="200px" className="object-cover" priority />
                </div>
                <div className="flex-1">
                  <span className="text-xs font-bold uppercase tracking-wider text-purple-400 flex items-center gap-1">
                    <Disc size={14} /> Album
                  </span>
                  <h1 className="text-2xl sm:text-4xl font-extrabold text-white mt-1">{album.title}</h1>
                  <p className="text-xs sm:text-sm text-white/70 mt-2 font-medium">
                    {album.totalTracks} Tracks • High Fidelity Bengali Recording
                  </p>
                ...
                {tracksToPlay.map((t, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="hidden sm:block text-xs text-white/60 truncate">{album.title}</div>
