...
export default function LibraryPage(): ReactNode {
  const [tab, setTab] = useState<'LIKED' | 'RECENT' | 'DOWNLOADS'>('LIKED')
  ...
  const likedTracks = useAudioPlayer((s) => s.likedTracks)
  ...
  const totalDurationMs = useMemo(() => {
    return likedTracks.reduce((acc, t) => acc + (t.durationMs || 180000), 0)
  }, [likedTracks])
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-2 border-b border-white/10 pb-3">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${
                    tab === 'RECENT' ? 'bg-white text-black' : 'text-white/60 hover:text-white'
                  }`}
                >
                  Recently Played ({playedHistory.length})
                </button>
                <button
                ...
                <div>
                  {playedHistory.length === 0 ? (
                    <div className="p-12 text-center bg-white/5 rounded-2xl border border-white/10">
                      ...
                      <p className="text-xs text-white/60 mt-1 max-w-sm mx-auto">
                        Songs you play will automatically appear here in your listening history.
                      </p>
                      <Link href="/search" className="inline-block mt-4 text-xs font-bold text-[#F59E0B] hover:underline">
                        Search and play music →
                      </Link>
                    </div>
                  ) : (
                    <div className="glass-card-strong overflow-hidden rounded-2xl divide-y divide-white/5">
                      {playedHistory.map((t, idx) => {
                        const playing = isCurrentPlaying(t.id)
                        const liked = likedTrackIds.includes(t.id)
                        return (
                          <div
                            key={`${t.id}-${idx}`}
                            onClick={() => play(t as any, playedHistory.map((tr) => ({ trackId: tr.id, track: tr as any })), idx)}
                            className="group grid grid-cols-[36px_1fr_minmax(80px,auto)] sm:grid-cols-[32px_1fr_1fr_80px_48px] gap-3 px-4 py-3 items-center hover:bg-white/5 cursor-pointer transition-colors"
                          >
                            <div className="flex items-center justify-center text-sm font-semibold text-white/50">
                              {playing ? (
                            ...
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="relative w-10 h-10 rounded-md overflow-hidden flex-shrink-0">
                                <Image src={t.album?.coverArtUrl || 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=600&auto=format&fit=crop&q=80'} alt={t.title} fill sizes="40px" className="object-cover" />
                              </div>
                              <div className="min-w-0">
                                <div className="text-sm font-semibold text-white truncate">{t.title}</div>
                                <div className="text-xs text-white/60 truncate">{t.artist?.name}</div>
                            ...
                            <div className="hidden sm:block text-xs text-white/60 truncate">{t.album?.title}</div>
                            <div className="text-xs text-white/50 text-right">{formatDuration(t.durationMs || 180000)}</div>
                            <div className="hidden sm:flex items-center justify-end">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation()
